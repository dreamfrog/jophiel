...
basedir=/Data/apps/mysql-5.0.41-icc/
datadir=/Data/apps/mysql-5.0.41-icc/data_23301

# 脚本优先从$datadir所在目录中查找my.cnf
# port和socket将从my.cnf中解析
# [mysqld]
# port=3307
...
case "$mode" in
  'start')
    ...
    then
      ...
      #echo "$bindir/mysqld_safe --datadir=$datadir --port=$port --pid-file=$server_pid_file $other_args >/dev/null 2>&1 &"
      $bindir/mysqld_safe --datadir=$datadir --port=$port --socket=$socket --pid-file=$server_pid_file $other_args >/dev/null 2>&1 &
      wait_for_pid created $!; return_value=$?
  ...
  'restart')
    ...
    if $0 stop  $other_args; then
      $0 start $other_args
    #if $script_full_path stop  $other_args; then
    #  $script_full_path start $other_args
    else
