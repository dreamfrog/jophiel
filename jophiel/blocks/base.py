...
from .models import RegisteredBlock
from . import params
from .items import RegistrableItem
from .meta import BlockBaseMetaclass


logger = getLogger('jophiel.blocks')
...
def _register_new_item(item_cls, activate):
    attributes = { 
        'class_name': item_cls.get_class_name(),
        'module': item_cls.get_module(),
    }   
    # Add attributes for extended class
    extended_attrs = item_cls.get_extended_attrs()
    attributes.update(extended_attrs)
    registered_item = item_cls.model(**attributes)
    registered_item.category = item_cls.get_category()
    registered_item.set_default_config(item_cls)
    if activate:
        registered_item.active = True
