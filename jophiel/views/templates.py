...
from jophiel import failure
import os
import datetime

from jophiel.redisclient import client as redis
...
class Overview(ResWeb):
    def __init__(self, queue=None, start=0):
        self._queue = queue
        self._start = start
...
class Stat(ResWeb):
    ...
    def size(self):
        return redis_size(self.stat_id)

class Worker(ResWeb):
    def __init__(self, worker_id):
        self.worker_id = worker_id
        super(Worker, self).__init__()
        self._worker = WorkerResource.find(worker_id)

    def worker(self):
        return str(self.worker_id)

    def host(self):
        hosts, pid, queues = str(self.worker_id).split(':')
        return str(hosts)
    ...
    def processed(self):
        return str(self._worker.processed())

    def failed(self):
        return str(self._worker.failed())
...
class DelayedTimestamp(ResWeb):
    def __init__(self, timestamp, start=0):
        self._start = start
        self._timestamp = timestamp
        super(DelayedTimestamp, self).__init__()
...
def redis_size(key):
    key_type = redis.type('resque:' + key)
    item = 0
    if key_type == 'list':
        item = redis.llen('resque:' + key)
    elif key_type == 'set':
        item = redis.scard('resque:' + key)
