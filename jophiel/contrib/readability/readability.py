#!/usr/bin/env python
from jophiel.contrib.BeautifulSoup import NavigableString
from jophiel.contrib.readability.page_parser import parse, get_title, get_body, Unparseable
import logging
import re
...
REGEXES = { 'unlikelyCandidatesRe': re.compile('combx|comment|disqus|foot|header|menu|meta|nav|rss|shoutbox|sidebar|sponsor',re.I),
    ...
    'divToPElementsRe': re.compile('<(a|span|blockquote|dl|div|img|ol|p|pre|table|ul)',re.I),
...
def main():
    ...
    try:
        ...
        summary = Document(file.read(), debug=options.verbose).summary()
        text = html2text.html2text(extractors.htmlregion(summary))
        print text
    finally:
        file.close()
