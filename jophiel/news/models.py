...
class Feed(models.Model):
    ...
    def update_entry(self,info):
        ...
        if info.has_key("modified_parsed") and info.modified_parsed:
            self.url_modified = utils.get_as_date(info.modified_parsed) 
    ...
    def update_articles(self, entries):
        ...
        for entry in entries:
            ...
            if not item:
                item = self.articles.create(id = entry_id) 
    ...
    def process_feed(self):
        ...
        self.save()
        feedmeta.update_entity(data)
        self.update_articles(data.entries)
...
class FeedMeta(models.Model):
    ...
    link = models.CharField(max_length=255,default = "") #Link to the original format feed (*).
    ...
    image_url = models.CharField(max_length=255,default="") # URL of an associated image (*).
    image_link = models.CharField(max_length=255,default="") #Link to go with the associated image (*).
                ...
                try:
                    ...
                    if feed.has_key(detail) and feed[detail].has_key('type'):
                        if feed[detail].type == 'text/html':
                            ...
                            feed[key] = utils.escape(feed[key])
...
class Article(models.Model):
    ...
    id = models.CharField(max_length=255,primary_key=True) #Channel-unique identifier for this item.
    ...
    link = models.CharField(max_length=255,default = "")   #Link to the original format text (*).
    ...
    source_link = models.CharField(max_length=255,blank = True) #   Link to the original source of this item (*).
    ...
    def get_entry_id(cls,url,entry):
        entry_id = None
        if entry.has_key("id"):
            entry_id = utils.utf8(entry.id)
        elif entry.has_key("link"):
            entry_id = utils.utf8(entry.link)
        elif entry.has_key("title"):
            entry_id = (url + "/" + md5.new(utils.utf8(entry.title)).hexdigest())
        elif entry.has_key("summary"):
            entry_id = (url + "/" + md5.new(utils.utf8(entry.summary)).hexdigest()) 
        return entry_id 
            ...
            if key in self.IGNORE_KEYS or key + "_parsed" in self.IGNORE_KEYS:
                ...
                for item in entry[key]:
                    if item.type == 'text/html':
                        ...
                        item.value = utils.escape(item.value)
                    ...
                    if entry.has_key(detail):
                        if entry[detail].has_key('type'):
                            if entry[detail].type == 'text/html':
                                ...
                                entry[key] = utils.escape(entry[key])
