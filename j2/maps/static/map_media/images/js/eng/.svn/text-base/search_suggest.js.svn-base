...
var choose_flag = "no";
var sunjg = "";
var regular_1 = /(<strong>)/g;
var regular_2 = /(<\/strong>)/g;
...
{ 
	...
	if(other_width_src == 32){other_width_list = 232;}
	...
	{ 
		...
		{
			document.getElementById(InputName+"search_suggest").innerHTML+="<div id='"+InputName+"item" + i + "' style=\"width:"+other_width_list+"\" class='item_normal' onmouseover='beMouseOver(" + i +",\""+InputName+"\")'  onclick='beClick(" + i + ",\""+InputName+"\")'>"+"<span>" + array[i] + "</span></div>"; 
		}
	...
	{ 
		...
		{
			document.getElementById(InputName+"search_suggest").innerHTML+="<div id='"+InputName+"item" + i + "' style=\"width:"+other_width_list+"\" align=right class='item_normal' onmouseover='beMouseOver(" + i +",\""+InputName+"\")'  onclick='beClick(" + i + ",\""+InputName+"\")'>" + "<span class=\"sclose\" onMouseOver=\"tdMOver(this)\" onMouseOut=\"tdMOut(this)\">"+"<u>"+array[i]+"</u></span>" + "</div>"; 
		}
	}
}//for循环
}//函数
...
function beMouseOverEFF(i,InputName){ 
	...
	{ 
		...
		var tmp_show = array[i];
		tmp_show = tmp_show.replace(regular_1,'');
		tmp_show = tmp_show.replace(regular_2,'');
		document.getElementById(InputName).value=tmp_show; 
	}
}
//函数鼠标移开效果 
...
function beOnBlur(InputName)
{
	...
	if(InputName == "area")
	{
		...
		{
			document.form1.area.value='city';
		}
	}
}
...
function beOnFocus(InputName)
{
	if(InputName == "area")
	{
		if(document.form1.area.value == "city")
		{
...
function SuggestShow(InputName,Url,input_width,other_info)
{
	...
	{
		other_width_src =input_width;
	}
	
	document.write("<div style='position:relative;z-index:200"+";'>");
	document.write("<input id=\""+InputName+"\" size="+input_width+" "+ other_info+" type=\"text\" name=\""+InputName+"\" class=\"key_normal\" onKeyDown=\"beKeyDown('"+InputName+"',"+"event"+")\" onKeyUp=\"beKeyUp('"+InputName+"','"+Url+'&name='+InputName+'&input='+"',"+"event"+")\" onfocus=\"beOnFocus('"+InputName+"')\" onblur=\"beOnBlur('"+InputName+"')\" autoComplete=\"off\"\"/>");
	//alert("<input id=\""+InputName+"\" size="+input_width+" "+ other_info+" type=\"text\" name=\""+InputName+"\" class=\"key_normal\" onKeyDown=\"beKeyDown('"+InputName+"',"+"event"+")\" onKeyUp=\"beKeyUp('"+InputName+"','"+Url+'&name='+InputName+'&input='+"',"+"event"+")\" onfocus=\"beOnFocus('"+InputName+"')\" onblur=\"beOnBlur('"+InputName+"')\" autoComplete=\"off\"\"/>");
